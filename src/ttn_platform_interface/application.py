...
from .app_ui import TtnPlatformInterfaceUI
from .app_state import TtnPlatformInterfaceState

log = logging.getLogger()

class TtnPlatformInterfaceApplication(Application):
    config: TtnPlatformInterfaceConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: TtnPlatformInterfaceUI = None
        self.state: TtnPlatformInterfaceState = None

    async def setup(self):
        self.ui = TtnPlatformInterfaceUI()
        self.state = TtnPlatformInterfaceState()
